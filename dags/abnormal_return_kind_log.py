...
# 테이블 및 컬럼 설정
TABLE_NAME = "abnormal_return_kind_log"
ABN_RET_COLUMNS = {
    **{-i: f"abn_ret_minus_{i}m" for i in range(1, 61)},
    0: "abn_ret_0m",
    **{i: f"abn_ret_{i}m" for i in range(1, 61)},
}
...
@dag(
    ...
    catchup=False,
    tags=["ticks", "event_returns", "log"],
    max_active_tasks=1,
)
def event_reaction_returns_kind_log_dag():
    @task
    ...
    def calculate_abnormal_returns_for_events(
        ...
        for ev in events:
            ...
            for i, lag in enumerate(lag_minutes):
                p_t = p_lags[i]
                market_p_t = market_p_lags[i]

                # 이전 시점 (t-1) 가격 조회
                ts_t_minus_1 = ts0 + timedelta(minutes=lag - 1)
                p_t_minus_1 = price_map.get((stock_code, ts_t_minus_1))
                market_p_t_minus_1 = market_price_map.get(ts_t_minus_1)
                ...
                if (
                    ...
                    and p_t_minus_1
                    and p_t_minus_1 > 0
                    and market_p_t_minus_1
                    and market_p_t_minus_1 > 0
                ):
                    # 개별 수익률 계산 (log return: log(P_t / P_{t-1}))
                    stock_return = np.log(p_t / p_t_minus_1)
                    # market 수익률 계산 (log return: log(M_t / M_{t-1}))
                    market_return = np.log(market_p_t / market_p_t_minus_1)
                    # abnormal return = 개별 수익률 - market 수익률
                    abnormal_return = stock_return - market_return
                    rec[ABN_RET_COLUMNS[lag]] = round(abnormal_return, 6)
...
dag = event_reaction_returns_kind_log_dag()
