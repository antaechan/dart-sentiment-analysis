...
# 테이블 및 컬럼 설정
TABLE_NAME = "volume_cum_kind_log"
VOLUME_COLUMNS = {
    **{-i: f"cum_volume_minus_{i}m" for i in range(1, 61)},
    **{i: f"cum_volume_{i}m" for i in range(1, 61)},
}
...
def event_reaction_cum_volume_kind_dag():
    @task
    ...
    def calculate_volumes_for_events(
        ...
        """이벤트 리스트에 대해 cumulative volume을 계산하는 헬퍼 함수"""
        updates = []
        ...
        for ev in events:
            ...
            for lag in lag_minutes:
                ...
                if lag < 0:
                    # 음수: -lag ~ 0 까지 합 (예: -60 ~ 0)
                    for minute in range(lag, 1):  # lag부터 0까지 (inclusive)
                        ts = ts0 + timedelta(minutes=minute)
                        vol = volume_map.get((stock_code, ts))
                        if vol is not None:
                            cum_volume += vol
                            has_data = True
                else:
                    # 양수: 0 ~ lag 까지 합 (예: 0 ~ 60)
                    for minute in range(0, lag + 1):  # 0부터 lag까지 (inclusive)
                        ts = ts0 + timedelta(minutes=minute)
                        vol = volume_map.get((stock_code, ts))
                        if vol is not None:
                            cum_volume += vol
                ...
                if has_data:
                    rec[VOLUME_COLUMNS[lag]] = cum_volume
...
dag = event_reaction_cum_volume_kind_dag()
